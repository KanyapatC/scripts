...
free -m | tee -a $motif_file_process
...
echo "\n==== OS Errors - Check all file:" | tee -a $motif_file_oslog
debsums -ae | tee -a $motif_file_oslog
echo "\n==== OS Errors - Check change file:" | tee -a $motif_file_oslog
debsums -as | tee -a $motif_file_oslog
 


/bin/echo "script has been finished successfully "

tar -zcvf "baculaserver-logs_$(date '+%Y-%m-%d').tar.gz" /app/logs/*
